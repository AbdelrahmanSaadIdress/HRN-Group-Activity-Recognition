...
class RCRG_3R_421C_conc_stage2(nn.Module):
    def __init__(self, 
                stage1_model, in_channels=2048,
                hidden_channels=512, output_channels=[512, 256, 128],
                num_classes=8
                ):
        super().__init__()
        ...
        self.relational_unit_three = RelationLayer(in_channels, hidden_channels, output_channels[2])

        # Example Stage 2 classifier
        self.classifier = nn.Sequential(
            nn.Linear(12*sum(output_channels), 2048),
            nn.BatchNorm1d(2048),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(2048, 1024),
    ...
    def forward(self, x):
        ...
        edge_index_3 = torch.tensor([(i, j) for i, j in itertools.permutations(range(P//4), 2)]).t().to(x.device)
        x_3_1 = self.relational_unit_three(input_=x[:,:3,:], edge_index=edge_index_3)
        x_3_2 = self.relational_unit_three(input_=x[:,3:6,:], edge_index=edge_index_3)
        x_3_3 = self.relational_unit_three(input_=x[:,6:9,:], edge_index=edge_index_3)
        x_3_4 = self.relational_unit_three(input_=x[:,9:,:], edge_index=edge_index_3)
        x_3 = torch.cat([x_3_1, x_3_2, x_3_3, x_3_4], dim=1)
        # (B, P, (output_channels_1+output_channels_2+output_channels_3))
...
# model = RCRG_3R_421C_conc_stage2(stage1_model)
# # Random input
# B = 2
# P = 12
